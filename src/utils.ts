...
class WrappedError implements Error {
     ...
     **/
    export var load_extensions = function (...exts: IPythonExtension[]): void {
     ...
     */
    export var load_extensions_from_config = function(section: ISection): void {
     ...
     * // -> ['..', 'word', '1', ' ', 'word', '2', '..']
     */
    export var regex_split = function (str: string, separator: MyRegExp, limit: number): string[] {
        var output: string[] = [],
            flags: string = (separator.ignoreCase ? "i" : "") +
                    (separator.multiline  ? "m" : "") +
                    (separator.extended   ? "x" : "") + // Proposed for ES6
                    (separator.sticky     ? "y" : ""), // Firefox 3+
            lastLastIndex: number = 0,
            separator2: RegExp, match: RegExpMatchArray, lastIndex: number, 
            lastLength: number;
        ...
        for (match = separator.exec(str); match; match = separator.exec(str)) {
            ...
            if (lastIndex > lastLastIndex) {
                ...
                if (!compliantExecNpcg && match.length > 1) {
                    match[0].replace(separator2, function (): string {
    ...
    export var ansi_colormap: { [s: string]: string; } = {
        "01":"ansibold",
    ...
    function _process_numbers(attrs: { style?: string; class?: string; }, numbers: string[]) {
        ...
        if (ansi_colormap[n]) {
            ...
            if (index_or_rgb == "5") {
                ...
                r = Number(numbers.shift());
                g = Number(numbers.shift());
                b = Number(numbers.shift());
            } else {
    ...
    export var ansispan = function(str: string): string {
        ...
        var is_open = false;
        return str.replace(/\033\[(0?[01]|22|39)?([;\d]+)?m/g, function(match, prefix, pattern): string {
            if (!pattern) {
    ...
    export var fixCarriageReturn = function(txt: string) {
        var tmp = txt;
        do {
            txt = tmp;
            tmp = txt.replace(/\r+\n/gm, '\n'); // \r followed by \n --> newline
    ...
    export var always_new = function (constructor: Function) {
        /**
    ...
    export var url_join_encode = function (...args: string[]):string {
        /**
         * join a sequence of url components with '/',
    ...
    export var to_absolute_cursor_pos = function (cm: CodeMirror.Editor, cursor: CodeMirror.Cursor):number {
        /**
    ...
    export var from_absolute_cursor_pos = function (cm: CodeMirror.Editor, cursor_pos: CodeMirror.Cursor) {
        /**
    ...
    export var is_or_has = function (a: any, b: any): boolean {
        /**
    ...
    export var is_focused = function (e: any) {
        /**
    ...
    export var mergeopt = function(_class: any, options: Object, overwrite: Object){
        options = options || {};
        overwrite = overwrite || {};
    ...
    export var requireCodeMirrorMode = function (mode: CodeMirror.Mode, callback: Function, errback: Function) {
        /** 
    ...
    export var promising_ajax = function(url: string, settings: any): Promise<any> {
        /**
    ...
    export var resolve_promises_dict = function(d: { [string: string]: Promise<any>; }) {
        /**
    ...
    export var reject = function(message: string, log?: boolean): Function {
        /**
    ...
    export var typeset = function(element: any, text: string) {
        /**
